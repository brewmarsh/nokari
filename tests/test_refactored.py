...
class RefactoredScraperTestCase(unittest.TestCase):
    ...
    def test_scrape_jobs_overrides_with_scraped_details(self, mock_build, mock_scrape_details):
        """
            ...
            'items': [
                {
                    ...
                    'pagemap': {
                        'metatags': [{'og:site_name': 'Initial Company'}]
                    }
                }
            ]
        }
        ...
        with patch.dict('os.environ', {'GOOGLE_API_KEY': 'fake_key', 'CUSTOM_SEARCH_ENGINE_ID': 'fake_id'}):
        ...
        self.assertEqual(job['description'], 'This is the full, detailed description from the website.')
