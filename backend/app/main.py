...
def create_job(job_request: models.CreateJobRequest, current_user: dict = Depends(get_current_user)):
    if "cognito:groups" not in current_user or "Admin" not in current_user["cognito:groups"]:
...
async def upload_resume(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to upload resume: {e}")
