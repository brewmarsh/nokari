...
async def test_create_job_as_non_admin(mock_dynamo_repo: MagicMock):
    ...
    job_data = {"title": "Test", "company": "Test", "location": "Test", "description": "Test", "work_arrangement": "Remote"}
...
async def test_get_job(mock_dynamo_repo: MagicMock):
    job_id = "test-job-id"
    mock_job_data = {"title": "Software Engineer", "company": "Tech Corp", "location": "Remote", "description": "A job opening.", "work_arrangement": "Remote"}
...
async def test_upload_resume(mock_s3_client: MagicMock, mock_dynamo_repo: MagicMock):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.post("/resumes/upload", files={"file": ("resume.txt", file_content, "text/plain")})
